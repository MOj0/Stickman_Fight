...
app.use(express.static(__dirname+"/server")); // Serve the client directory
...
setInterval(heartbeat, 10);     // Milliseconds
...
function heartbeat() {
  ...
  io.sockets.emit('heartbeat', players, lasersAll, spawns, items); // Broadcasts updated inforamtion to all clients
  lasersAll.length = 0;
...
io.sockets.on('connection',
  function(socket) {
    ...
    socket.on('update', function(data, lasers) {
    ...
    socket.on('shoot', function(playerLaser) {
        ...
        lasersAll.push(laser);
      }
    );

    socket.on('useItem', function(item) {
      for (var i = 0; i < players.length; i++) {
        if (socket.id === players[i].id) {
          if (item === 5) {               // Increase Max Health
            console.log("Player UID: " + players[i].uid);
            var newMaxLife = players[i].maxLife + 20; //
            players[i].maxLife = newMaxLife; // Sets new maxLife on player object(server)
            io.sockets.to(players[i].id).emit('updateMaxLife', newMaxLife); // Updates player.maxLife at client side
            let writeData = {
              maxLife: newMaxLife,
            };
            db.collection('players').where('uid', '==', players[i].uid).get().then((data) => {
                var documents = data.docs;
                if (documents.length == 1) {
                    var id = documents[0].id;

                    db.collection('players').doc(id).update(writeData).then(() => {
                        console.log("New maxLife written to database!");
                    });
                } else {
                  console.error("Multiple same UIDs!");
                }
            });
          }
          else if (item === 6) {
            console.log("Teleporting player: " + players[i].player);
            updatePlayer(players[i].id, random(300, 4200), random(300, 4200), players[i].life);
          }
